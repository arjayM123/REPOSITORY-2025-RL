...
$stmt = $pdo->prepare("SELECT COUNT(*) as total FROM book_requests");
$stmt->execute();
$totalBookRequests = $stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;

// Get all book requests
$stmt = $pdo->prepare("SELECT * FROM book_requests ORDER BY requested_at DESC");
$stmt->execute();
$bookRequests = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
if ($view === 'active') {
    ...
    $whereConditions[] = "is_deleted = 0 AND status = 'locked'";
    $orderBy = " ORDER BY created_at DESC";
} elseif ($view === 'popular') {
    $whereConditions[] = "is_deleted = 0";
...
try {
    ...
    $query .= isset($orderBy) ? $orderBy : " ORDER BY created_at DESC";
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="mb-0 text-primary fw-bold">
        <i class="bi bi-collection me-2"></i>Books Management
    </h2>
    ...
    <div class="col-md-3 mb-3">
        <div class="card bg-info text-white text-center h-100">
            <div class="card-body">
                ...
                <h3 class="fw-bold"><?php echo number_format($totalBookRequests); ?></h3>
                <p class="mb-0">Total Book Requests</p>
            </div>
        </div>
    </div>
</div>
...
<ul class="nav nav-pills mb-4">
    ...
    <li class="nav-item">
        <a class="nav-link <?php echo $view === 'favorite' ? 'active' : ''; ?>" href="#"
            onclick="changeView('favorite')">
            <i class="bi bi-star me-1"></i>Favorite
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link <?php echo $view === 'book_request' ? 'active' : ''; ?>" href="#"
            onclick="changeView('book_request')">
        ...
        <form method="GET" class="row g-3">
            ...
            <div class="col-md-2">
                <select name="type_material" class="form-select">
                    ...
                    <option value="Magazine" <?php echo $type_material === 'Magazine' ? 'selected' : ''; ?>>Magazine
                    </option>
                    <option value="Newspaper" <?php echo $type_material === 'Newspaper' ? 'selected' : ''; ?>>Newspaper
                    </option>
                    <option value="Reference" <?php echo $type_material === 'Reference' ? 'selected' : ''; ?>>Reference
                    </option>
            ...
            <div class="col-md-3">
                <div class="d-flex gap-2">
                    ...
                    <a href="?page=manage-books&view=<?php echo htmlspecialchars($view); ?>"
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php foreach ($bookRequests as $i => $req): ?>
                                <tr>
                                    <td><?php echo $i + 1; ?></td>
                                    <td><?php echo !empty($req['student_name']) ? htmlspecialchars($req['student_name']) : 'N/A'; ?>
                                    </td>
                                    <td><?php echo !empty($req['id_number']) ? htmlspecialchars($req['id_number']) : 'N/A'; ?></td>
                                    <td><?php echo !empty($req['book_title']) ? htmlspecialchars($req['book_title']) : 'N/A'; ?>
                                    </td>
                                    <td><?php echo !empty($req['book_author']) ? htmlspecialchars($req['book_author']) : 'N/A'; ?>
                                    </td>
                                    ...
                                    <td><?php echo !empty($req['requested_at']) ? date('M d, Y H:i', strtotime($req['requested_at'])) : 'N/A'; ?>
                                    </td>
<td>
    <button type="button" 
            class="btn btn-sm btn-danger" 
            onclick="showDeleteRequestModal(<?php echo $req['id']; ?>)">
                ...
                <div class="col-lg-4 col-md-6 mb-4">
                    <div class="card h-100 shadow-sm position-relative book-card"
                        ...
                        <div class="position-absolute top-0 end-0 m-2" style="z-index: 1000;">
                            <div class="dropdown">
                                <button class="btn btn-light btn-sm dropdown-toggle shadow-sm" type="button"
                                    data-bs-toggle="dropdown" data-bs-auto-close="outside" onclick="event.stopPropagation()">
                                    <i class="bi bi-three-dots-vertical"></i>
                                </button>
                                <ul class="dropdown-menu dropdown-menu-end shadow">
                                    <?php if ($view !== 'deleted'): ?>
                                        <li>
                                            <a href="?page=edit_book&id=<?php echo $book['id']; ?>" class="dropdown-item"
                        ...
                        <div class="text-center pt-3">
                            ...
                            ?>
                            <img src="<?php echo htmlspecialchars($coverImage); ?>" class="img-fluid rounded"
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-2">
                                <small class="text-muted">
                                    ...
                                    <?php echo (int) $book['view_count']; ?> views
                                </small>
                                <small class="text-muted">
                                    <i class="bi bi-star me-1"></i>
                                    <?php echo (int) $book['favorite_count']; ?> favorites
                                </small>
                            </div>
                        </div>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-4 text-center">
                        <img id="modal-cover" src="../assets/images/genericBookCover.jpg" class="img-fluid rounded mb-3"
                            style="max-height: 300px;" onerror="this.src='../assets/images/genericBookCover.jpg'">
                    </div>
                    <div class="col-md-8">
                        <table class="table table-borderless">
                            ...
                            <tr>
                                <th>Copies:</th>
                                <td id="modal-copies"></td>
                            </tr>
                            <tr>
                                <th>Classification:</th>
                                <td id="modal-classification"></td>
                            </tr>
                            <tr>
                                <th>Call Number:</th>
                                <td id="modal-call-number"></td>
                            </tr>
                            <tr>
                                <th>Accession Number:</th>
                                <td id="modal-accession"></td>
                            </tr>
                        </table>
                        <div class="mt-3">
                            <h6>Description:</h6>
                            ...
                            <?php if (!empty($stats['by_department'])): ?>
                                <?php foreach ($stats['by_department'] as $dept): ?>
                                    <div class="list-group-item d-flex justify-content-between align-items-center">
                                        ...
                                        <span
                                            class="badge bg-primary rounded-pill"><?php echo number_format($dept['count']); ?></span>
                                    </div>
                                <?php endforeach; ?>
                            <?php else: ?>
                    ...
                    <div class="col-md-6">
                            ...
                            <?php if (!empty($stats['by_material'])): ?>
                                <?php foreach ($stats['by_material'] as $material): ?>
                                    <div class="list-group-item d-flex justify-content-between align-items-center">
                                        ...
                                        <span
                                            class="badge bg-info rounded-pill"><?php echo number_format($material['count']); ?></span>
                                    </div>
                                <?php endforeach; ?>
                            <?php else: ?>
                        ...
                        <div class="list-group list-group-flush">
                            <div class="list-group-item d-flex justify-content-between align-items-center">
                                Active Books
                                <span
                                    class="badge bg-success rounded-pill"><?php echo number_format($stats['by_status']['active']); ?></span>
                            </div>
                            <div class="list-group-item d-flex justify-content-between align-items-center">
                                Locked Books
                                <span
                                    class="badge bg-warning rounded-pill"><?php echo number_format($stats['by_status']['locked']); ?></span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="alert alert-danger mb-0">
                    ...
                    <small><strong>Warning:</strong> This action cannot be undone! The book will be completely removed
                        from the system.</small>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
...
<!-- Update the deleteRequestModal form action -->
...
<div class="modal fade" id="deleteRequestModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-danger text-white">
                <h5 class="modal-title">
                    <i class="bi bi-exclamation-triangle me-2"></i>Delete Request
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <p>Are you sure you want to delete this book request?</p>
...
function showDeleteRequestModal(requestId) {
    // Set the request ID in the hidden form field
    document.getElementById('delete-request-id').value = requestId;
    ...
    document.getElementById('deleteRequestForm').onsubmit = function() {
